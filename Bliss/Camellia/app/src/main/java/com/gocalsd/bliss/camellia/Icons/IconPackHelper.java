...
import com.gocalsd.bliss.camellia.Models.IconData;
...
public class IconPackHelper implements OnDismissListener {
            ...
            if (!TextUtils.isEmpty(item)) {
                ...
                if (id != 0) {
                    return ResourcesCompat.getDrawable(mLoadedIconPackResource, id, mContext.getTheme());
                            //mLoadedIconPackResource.getDrawable(id);
                }
            }
        }
        return null;
    ...
    public boolean getThemedIcon(Context context, String packageName){
        return SettingsProvider.get(context).getBoolean(packageName + ".LauncherThemedIcon", true);
    }

    public void setThemedIcon(Context context, String packageName, boolean value){
        SettingsProvider.putBoolean(context,packageName + ".LauncherThemedIcon", value);
    }
    ...
    private void init(SharedPreferences prefs) {
        mCurrentIconPack = prefs.getString(SettingsProvider.PREF_ICONPACK, "");
        if (!TextUtils.isEmpty(mCurrentIconPack)){
            try {
                ...
                prefs.edit().putString(SettingsProvider.PREF_ICONPACK, "").apply();
